...
class ImageStandardizer(object):
    ...
    def fit(self, X):
        ...
        self.image_mean = np.mean(X, (0,1,2), dtype=np.float64)
        self.image_std = np.std(X, (0,1,2), dtype=np.float64)

    def transform(self, X):
        """Return standardized dataset given dataset X."""
        # TODO: Complete this function
        X_transformed = X - self.image_mean
        X_transformed = X_transformed/self.image_std
